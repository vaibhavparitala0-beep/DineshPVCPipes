...
const StaffDetailDialog = ({ staff, open, onClose }: StaffDetailDialogProps) => {
  const { 
  ...
  const attendanceStats = useMemo(() => {
    ...
    const periodRecords = attendanceRecords.filter(r => r.date >= cutoffString);
    const totalDays = periodRecords.length;
    const presentDays = periodRecords.filter(r => r.status === 'present').length;
    const lateDays = periodRecords.filter(r => r.status === 'late').length;
    const absentDays = periodRecords.filter(r => r.status === 'absent').length;
    const overtimeDays = periodRecords.filter(r => r.overtimeHours > 0).length;
    
    const totalHours = periodRecords.reduce((sum, r) => sum + r.totalHours, 0);
    const totalOvertimeHours = periodRecords.reduce((sum, r) => sum + r.overtimeHours, 0);
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          <div className="text-gray-600">
                            ...
                            <p>{staff.address.city}, {staff.address.state} {staff.address.zipCode}</p>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-gray-600">{staff.emergencyContact.name}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-sm text-gray-500">Relationship:</span>
                          <span className="text-gray-600">{staff.emergencyContact.relationship}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Phone className="h-4 w-4 text-gray-500" />
                          <span className="text-gray-600">{staff.emergencyContact.phone}</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <p className="font-medium">{staff.department.replace('_', ' ')}</p>
                  </div>
                  
                  <div>
                    <p className="text-sm text-gray-600">Role</p>
                    <p className="font-medium">{staff.role.replace('_', ' ')}</p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {staff.workingHours.startTime} - {staff.workingHours.endTime}
                    </p>
                ...
                <CardContent>
                  {todayRecord ? (
                    <div className="flex items-center gap-6">
                      <Badge className={getAttendanceStatusColor(todayRecord.status)}>
                        {todayRecord.status.replace('_', ' ').toUpperCase()}
                      </Badge>
                      {todayRecord.clockIn && (
                        <div>
                          <p className="text-sm text-gray-600">Clock In</p>
                          <p className="font-medium">{formatTime(todayRecord.clockIn)}</p>
                        </div>
                      )}
                      {todayRecord.clockOut && (
                        <div>
                          <p className="text-sm text-gray-600">Clock Out</p>
                          <p className="font-medium">{formatTime(todayRecord.clockOut)}</p>
                        </div>
                      )}
                      {todayRecord.totalHours > 0 && (
                        <div>
                          <p className="text-sm text-gray-600">Total Hours</p>
                          <p className="font-medium">{todayRecord.totalHours.toFixed(1)}h</p>
                        </div>
                      )}
                    </div>
                  ) : (
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xl font-bold text-gray-900">{attendanceStats.presentDays}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xl font-bold text-gray-900">{attendanceStats.avgHours.toFixed(1)}h</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xl font-bold text-gray-900">{attendanceStats.overtimeDays}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div className="text-sm font-medium min-w-[60px] text-right">
                              ...
                              {record.overtimeHours > 0 && (
                                <p className="text-purple-600">+{record.overtimeHours.toFixed(1)}h OT</p>
                              )}
                            </div>
                          </div>
                        </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-400">Goals, reviews, and performance analytics</p>
                </div>
              </CardContent>
            </Card>
          </TabsContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-400">Contracts, certifications, and other documents</p>
                </div>
              </CardContent>
            </Card>
          </TabsContent>
