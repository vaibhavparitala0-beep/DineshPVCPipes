...
export const StaffProvider = ({ children }: { children: ReactNode }) => {
  ...
  const stats = useMemo(() => {
    ...
    const newHires = staff.filter((s) =>
      s.hireDate.startsWith(thisMonth),
    ).length;
    ...
    const avgAttendance =
      activeStaff > 0 ? (presentToday / activeStaff) * 100 : 0;
  ...
  const addStaff = async (staffData: StaffFormData): Promise<void> => {
    ...
    try {
      ...
      const selectedRoles = mockRoles.filter((role) =>
        staffData.roles.includes(role.id),
      );
          ...
          if (action.value) {
            for (const staffId of action.staffIds) {
              await updateStaffStatus(
                staffId,
                action.value as EmploymentStatus,
              );
            }
          }
          break;
