...
export class PDFExporter {
  ...
  private addCompanyHeader(title: string) {
    ...
    this.doc.setFont("helvetica", "normal");
    this.doc.text(`Generated on: ${new Date().toLocaleString()}`, this.margin, 70);
  ...
  private getStatusColor(status: OrderStatus): [number, number, number] {
    switch (status) {
      case "pending": return [255, 235, 59]; // Yellow
      case "confirmed": return [33, 150, 243]; // Blue
      case "processing": return [156, 39, 176]; // Purple
      case "shipped": return [33, 150, 243]; // Blue
      case "delivered": return [76, 175, 80]; // Green
      case "cancelled": return [244, 67, 54]; // Red
  ...
  private getPriorityColor(priority: Priority): [number, number, number] {
    switch (priority) {
      case "urgent": return [244, 67, 54]; // Red
      case "high": return [255, 152, 0]; // Orange
      case "medium": return [255, 235, 59]; // Yellow
  ...
  public exportOrdersReport(
    ...
    this.addFooter();
    this.doc.save(`orders-report-${new Date().toISOString().split('T')[0]}.pdf`);
  }
  ...
  private calculateOrderStats(orders: Order[]) {
    return {
      ...
      processing: orders.filter(o => ['confirmed', 'processing', 'manufacturing'].includes(o.status)).length,
  ...
  private calculateStaffStats(staff: Staff[]) {
    return {
      ...
      avgSalary: staff.length > 0 ? staff.reduce((sum, s) => sum + s.salary, 0) / staff.length : 0,
  ...
  private addAttendanceSummary(records: AttendanceRecord[], staff: Staff[]): void {
    ...
    const attendanceByStaff = records.reduce((acc, record) => {
    ...
    const attendanceData = Object.entries(attendanceByStaff).map(([staffId, staffRecords]) => {
      const staffMember = staff.find(s => s.id === staffId);
      const totalHours = staffRecords.reduce((sum, record) => sum + record.totalHours, 0);
...
export const exportOrdersReport = (orders: Order[], options?: ExportOptions) => {
  const exporter = new PDFExporter();
  exporter.exportOrdersReport(orders, options);
};

export const exportStaffReport = (staff: Staff[], attendanceRecords?: AttendanceRecord[], options?: ExportOptions) => {
  const exporter = new PDFExporter();
  exporter.exportStaffReport(staff, attendanceRecords, options);
};

export const exportSelectedOrders = (orderIds: string[], allOrders: Order[]) => {
  const exporter = new PDFExporter();
