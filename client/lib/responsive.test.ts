  ...
  const tests = [
    {
      ...
      test: () => RESPONSIVE_COMPONENTS.mobileNavButton.className === 'lg:hidden',
      description: 'Mobile nav button should be hidden on large screens'
    },
    {
      name: 'Desktop Navigation', 
      test: () => RESPONSIVE_COMPONENTS.desktopNav.className === 'hidden lg:block',
      description: 'Desktop nav should be hidden on mobile screens'
    },
    {
      name: 'Dashboard Grid',
      test: () => CARD_LAYOUTS.dashboard.main.includes('xl:grid-cols-3'),
      description: 'Dashboard should use 3-column layout on extra large screens'
    },
    {
      name: 'Metrics Grid',
